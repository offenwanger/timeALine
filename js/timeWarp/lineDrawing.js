let createLineDrawer = function (svg) {
    ...
    function onDragEnd() {
        if (isDrawing) {
            let result = [];
            for (let len = 0; len < drawingLine.node().getTotalLength(); len += 50) {
                result.push(drawingLine.node().getPointAtLength(len));
            }
            result.push(drawingLine.node().getPointAtLength(drawingLine.node().getTotalLength()));
